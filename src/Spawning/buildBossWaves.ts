...
export function buildBossWaves(
    ...
    for (let index = 0; index < locationList.length; index++) {
        ...
        if (disableBosses && !!locationList[index].base?.BossLocationSpawn) {
            ...
            locationList[index].base.BossLocationSpawn = locationList[index].base.BossLocationSpawn.filter(
                (boss) => !bossesToRemoveFromPool.has(boss.BossName)
            );

            // Performance changes
            if (advancedConfig.EnableBossPerformanceImprovements) {
                locationList[index].base.BossLocationSpawn.forEach((Boss, bIndex) => {
                    ...
                    if (bossPerformanceHash[Boss.BossName || ""]) {
                        ...
                        locationList[index].base.BossLocationSpawn[bIndex] = {
                            ...Boss,
                            ...varsToUpdate
                        };
                    }
            ...
            const defaultBossSettings =
                mapConfig?.[configLocations[index]]?.defaultBossSettings;
            ...
            if (randomRaiderGroup) {
                const raiderWave = buildBossBasedWave(
                    ...
                    "pmcBot",
                    "",
                    locationList[index].base.EscapeTimeLimit
                );
